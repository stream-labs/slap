  ...
  constructor(
    ...
    Object.keys(defaultState).forEach(propertyName => {
      ...
      if (!config.mutations[removeMutation]) {
        controller.registerMutation(removeMutation, (searchFn: (item: unknown) => boolean) => {
          const arr = (controller as any)[propertyName];
          let i = arr.length;
          while (i--) {
...
export type PickAutogeneratedRemoveMutations<TConfigCreator, TState = TStateFor<TConfigCreator>> = {
  [K in keyof TState as GetRemoveMutationName<K>]: TState[K] extends Array<infer TItem> ? (searchFn: (newItem: TItem) => boolean) => void : never;
}
