...
export class Store {
  ...
  createState<TConfigCreator>(moduleName: string, configCreator: TConfigCreator): GetStateControllerFor<TConfigCreator> {
    ...
    this.events.emit('onModuleCreated', moduleName);
...
export interface StoreEvents {
  ...
  onMutation: (moduleName: string, mutation: Mutation | null) => void
...
export class StateController<TConfig = any> {
  ...
  mutate(mutation: ((draft: this) => unknown) | Mutation) {
    ...
    this.store.events.emit('onMutation', moduleName, mutationIsFunction ? null : mutation);
